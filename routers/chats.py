...
async def create_chat(
    body: Dict[str, Any] = Body(
        ...
        examples={
            "with_message": {
                ...
                "value": {"title": "My chat", "message": "Hello world", "agent_id": None},
            },
            "empty": {
                "summary": "Create empty chat",
                "value": {"title": "My new chat"},
            }
        },
    ),
    authorization: str = Header(..., alias="Authorization"),
):
    """Create a new chat. Body: {"title": "optional", "message": "optional", "agent_id": "optional"}
    
    If message is provided, it will be processed as the first message in the chat.
    If no message is provided, an empty chat will be created.
    """
    if not authorization or not authorization.lower().startswith("bearer "):
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Missing or invalid Authorization header")
    token = authorization.split(" ", 1)[1].strip()
    ...
    # If message is provided, process it as the initial message
    if message:
